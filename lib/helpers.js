...
export function resetSite (siteUrl) {
    ...
    const res = http.post(`${siteUrl}/wp-json/k6/v1/reset`, null, {
        headers: { 'X-K6-Secret': secret },
    })

    if (res.status !== 200) {
        exec.test.abort(`Site reset failed (${res.status}): ${res.body}`)
    }

    const { flushed } = res.json()
    console.log(`Site reset: ${flushed.join(', ')}`)
}
...
export function wpSitemap (url) {
    ...
    const unwrapCDATA = function (loc) {
        return loc.replace(new RegExp('<!-*\\[CDATA\\[(.+)]]-*>'), '$1')
    }
    ...
    response.html().find('url loc').each((idx, el) => {
        urls.push(unwrapCDATA(el.innerHTML()))
    })

    response.html().find('sitemap loc').each((idx, el) => {
        const loc = unwrapCDATA(el.innerHTML())
        sitemaps.push(loc)

        const response = http.get(loc)
