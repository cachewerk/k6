...
export default class Metrics {
    ...
    parseMetricsFromResponse (response) {
        ...
        const client = comment[0].toLowerCase().match(/client=(.+?) /)?.[1];

        metrics['using-relay'] = client ? client.includes('relay') : false
        metrics['using-phpredis'] = client ? client.includes('phpredis') : false
