...
export default class Metrics {
    constructor () {
        this.metrics = {
            ...
            'sys-load': new Trend('wp_sys_load'),
            ...
            'using-predis': new Rate('x_using_predis'),
            'using-apcu': new Rate('x_using_apcu'),
    ...
    parseMetricsFromResponse (response) {
        ...
        const comment = response.body.match(/<!-- plugin=[^\s]+ client=[^\s]+ (.+?) -->/g)
        ...
        metrics['using-predis'] = clientName.includes('predis')
        metrics['using-apcu'] = clientName.includes('apcu')
